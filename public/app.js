  ...
  openModal(`
    <section class="modal-card">
      ...
      <div class="settings-form">
        <label><span>API URL</span><input id="modalApiUrl" value="${attr(state.settings.apiUrl)}" placeholder="${attr(defaultApiUrl)}" /></label>
        ...
        <div class="settings-help">请使用 ccoder 内测分配的 API Key</div>
...
function normalizeSettings(value) {
  ...
  const apiUrl = normalizeApiBaseUrl(settings.apiUrl);
  return {
    ...settings,
    apiUrl: !settings.apiKey && apiUrl === legacyDefaultApiUrl ? defaults.settings.apiUrl : apiUrl,
